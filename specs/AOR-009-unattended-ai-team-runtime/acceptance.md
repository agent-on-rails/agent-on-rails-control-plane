...
- [ ] Every orchestration wave emits a **run report** whose JSON includes all required keys (`spec_id`, `terminal_status`, `completed`, `blocked_environment`, `skipped_due_to_deps`, `coding_failures`, `human_actions_needed`, `notifications_sent`) and validates against `schemas/evidence.schema.json` `run_report` (empty arrays allowed; omitted keys fail).
- [ ] Engine is the sole author of the run report; runtime/CLI contribute via events and may only fetch/display it.
- [ ] With `AOR_ENGINE_URL` configured, `aor run TASK-001` registers the task if needed (`POST /v1/tasks`) then uses live task APIs (`POST/GET /v1/tasks/{id}/...`) and does not fail due to missing endpoints.
- [ ] `POST /v1/tasks/{task_id}/run` returns 404 for an unknown `task_id` (no implicit invent-from-URL state).
